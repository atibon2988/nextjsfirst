      ...
      <div className="max-w-7xl mx-auto px-4 flex justify-between items-center gap-4">
        ...
        <div className="flex-1 max-w-md relative hidden md:block">
          <input
            ...
            className="w-full bg-slate-200/50 dark:bg-slate-800/50 border border-transparent focus:border-blue-500 rounded-full py-2 pl-10 pr-4 text-slate-900 dark:text-white outline-none transition-all"
          />
        ...
        <div className="flex items-center gap-3">
          <button className="flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-full text-sm font-bold hover:bg-blue-700 transition">
          ...
          {/* NÚT DARKMODE (Đã fix màu icon) */}
          <button
            onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}
            className="p-2 rounded-full bg-slate-200 dark:bg-slate-800 text-slate-800 dark:text-yellow-400 border border-slate-300 dark:border-slate-700 transition-all"
          >
            {theme === 'dark' ? (
              ...
              <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 20 20"><path d="M17.293 13.293A8 8 0 016.707 2.707a8.01 8.01 0 0010.586 10.586z" /></svg>
            )}
          </button>
        </div>
      </div>
