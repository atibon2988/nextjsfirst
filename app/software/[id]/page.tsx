...
export default async function DetailPage({ params }: Props) {
  ...
  return (
    <main className="min-h-screen bg-slate-50 pb-20">
      ...
      {/* 1. Header (Vẫn giữ header điều hướng) */}
      <header className="bg-white/90 backdrop-blur-md border-b py-4 px-6 sticky top-0 z-30 shadow-sm">
        <div className="max-w-4xl mx-auto flex justify-between items-center">
            <Link href="/" className="text-gray-600 hover:text-blue-600 flex items-center gap-2 text-sm font-medium transition">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
                Quay lại trang chủ
            </Link>
            <span className="font-bold text-blue-600 text-lg">KhoTaiNguyen</span>
        </div>
      </header>
