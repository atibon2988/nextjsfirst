  ...
  return (
    <header className={`fixed top-0 w-full z-50 transition-all duration-300 ${
      ...
      <div className="max-w-7xl mx-auto px-4 flex justify-between items-center gap-4">
        ...
        {/* SEARCH BAR - Sửa để không bị 404 */}
        <form onSubmit={handleSearch} className="flex-1 max-w-md relative hidden md:block">
          ...
          />
          <button type="submit" className="absolute left-3 top-2.5 text-slate-500">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative" ref={menuRef}>
            ...
            {isMenuOpen && (
              <div className="absolute right-0 mt-3 w-56 bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-100 dark:border-slate-700 py-3 z-50 animate-in fade-in zoom-in duration-200">
                <div className="px-4 py-2 text-xs font-bold text-slate-400 uppercase tracking-widest">Tài nguyên</div>
                <Link href="/?cat=Game" className="block px-4 py-2.5 hover:bg-blue-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 transition">🎮 Trò chơi</Link>
                <Link href="/?cat=Software" className="block px-4 py-2.5 hover:bg-blue-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 transition">💻 Phần mềm</Link>
                <Link href="/?cat=Document" className="block px-4 py-2.5 hover:bg-blue-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 transition">📚 Tài liệu học tập</Link>
                <div className="h-px bg-slate-100 dark:bg-slate-700 my-2"></div>
                <div className="px-4 py-2 text-xs font-bold text-slate-400 uppercase tracking-widest">Cộng đồng</div>
                <Link href="/?cat=News" className="block px-4 py-2.5 hover:bg-blue-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 transition">📰 Tin tức công nghệ</Link>
                <Link href="/?cat=Tips" className="block px-4 py-2.5 hover:bg-blue-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 transition">💡 Thủ thuật & Mẹo</Link>
              </div>
            )}
          </div>
          ...
          <button
            ...
            className="p-2.5 rounded-full bg-slate-200 dark:bg-slate-800 text-slate-800 dark:text-yellow-400 border border-slate-300 dark:border-slate-700 hover:scale-105 transition-all"
          >
            {theme === 'dark' ? (
